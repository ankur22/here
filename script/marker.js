  ...
  xhttp.onreadystatechange = function() {
    if (xhttp.readyState == XMLHttpRequest.DONE && xhttp.status == 200) {
      ...
      for (i = 0; i < greetings.length; ++i) {
        ...
        var photoDT = new Date(getDateFromFormat(greeting.photoDT, "yyyy:MM:dd HH:mm:ss"));
        var diff = Math.abs(photoDT - now) / 36e5;
        if (diff <= 12) {
          photoDT = parseInt(diff) + " hours ago";
        } else if (diff <= 24) {
          photoDT = "Today";
        } else if (diff <= 48) {
          photoDT = "Yesterday";
        } else {
          photoDT = greeting.photoDT;
        }

        contentStrings.push("<b>" + photoDT + "</b><br /><a href=\""+ greeting.photo +"\" target=\"_blank\"><img src=\""+ greeting.thumbnail +"\" /></a><p><div width=\"200\">" +greeting.notes+"</div></p><p>Photo uploaded on: "+greeting.eventDT+"</p>");
